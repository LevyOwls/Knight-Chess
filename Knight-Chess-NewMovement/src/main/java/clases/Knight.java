...
public class Knight 
{
		...
		for (i=0;i<8;i++)
		{
			...
			switch (i) 
			{
				case 0:
					...
					if (auxX<0 || auxX>7 || auxY<0 || auxY>7)//(-1<auxX && auxX<8 && -1<auxY && auxY<8)
					{
						break;
					}
					else
	...
	public Knight copyKnight()
	{
		int id=new Integer(getId());
		int x=new Integer(getX());
		int y=new Integer(getY());
		boolean enemy=isEnemy();
		Knight nuevo=new Knight(id,x,y,enemy);
		return nuevo;
	}
}
